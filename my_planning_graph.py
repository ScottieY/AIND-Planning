...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in actions:
            ...
            precond = node.precond_s_nodes()
            if precond.issubset(s_level):
                self.a_levels[level].add(node)
                for cond in s_level:
                    if cond in precond:
                        cond.children.add(node)
                node.parents.update(precond)
    ...
    def add_literal_level(self, level):
        ...
        for a_node in a_level:
            effect_nodes.update(a_node.effect_s_nodes())
        for s_node in effect_nodes:
            for a_node in a_level:
                if s_node in a_node.effect_s_nodes():
                    s_node.parents.add(a_node)
                    a_node.children.add(s_node)
        self.s_levels[level].update(effect_nodes)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for add_eff in node_a2.action.effect_add:
            if add_eff in node_a1.action.effect_rem:
                return True
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for c1 in node_a1.parents:
            for c2 in node_a2.parents:
                if c1.is_mutex(c2):
                    return True
        return False
    ...
    def h_levelsum(self) -> int:
        ...
        for level in range(len(self.s_levels)):
            achieved = goals.intersection(self.s_levels[level])
            level_sum += len(achieved) * level
